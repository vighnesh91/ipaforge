...
REQUIREMENTS
------------
Python 3.6+ and NOTHING else. The source retains the legacy plist/conversion
compatibility architecture from the original release; Python 2.7/3.2/3.3
syntax support is not claimed because the current engine uses Python 3 type
annotations and dataclass-style records. Standard library
...
try:
    ...
    # Python 3.6 compatibility: provide the tiny subset used by IPAForge.
    def dataclass(_cls=None, frozen=False, **_kwargs):
        def decorate(cls):
            fields = getattr(cls, "__annotations__", {})
            names = list(fields.keys())
            defaults = dict((name, getattr(cls, name)) for name in names
                            if hasattr(cls, name))
            def __init__(self, *args, **kwargs):
                ...
                if kwargs:
                    raise TypeError("unexpected arguments: %s" % ", ".join(kwargs.keys()))
            cls.__init__ = __init__
            if frozen:
                def _blocked(*_args, **_kw):
                    raise AttributeError("cannot assign to field of frozen dataclass")
                cls.__setattr__ = _blocked
                cls.__delattr__ = _blocked
            return cls
        return decorate(_cls) if _cls is not None else decorate
    def asdict(obj):
        return dict((name, getattr(obj, name))
                    for name in getattr(obj.__class__, "__annotations__", {}))
...
def _atomic_replace(src, dst):
    """Replace dst atomically where supported, with an old-Python fallback."""
...
class AnalysisPluginRegistry:
    ...
    def __init__(self):
        ...
        self.register("on_plist", _anti_analysis_detection_rule)
        self.register("on_binary", _anti_analysis_detection_rule)
...
_ANTI_ANALYSIS_MARKERS = (
    "Cydia", "cydia://", "frida-server", "frida-agent", "libfrida",
    "libjailbreak", "MobileSubstrate", "SubstrateLoader",
    "/Applications/Cydia.app", "/private/var/lib/apt",
    "/usr/sbin/sshd", "/bin/bash", "/usr/libexec/ssh-keysign",
    "checkra1n", "unc0ver", "TaigOne",
)


def _anti_analysis_detection_rule(path, raw, parsed=None, **_kwargs):
    """Scan plists and binaries for jailbreak / anti-analysis markers."""
    findings = []
    haystack = None
    if parsed is not None and isinstance(parsed, dict):
        ...
        try:
            haystack = bytes(raw).decode("utf-8", "ignore")
        except Exception:
            haystack = ""
    if not haystack:
        return findings
    lowered = haystack.lower()
    for indicator in _ANTI_ANALYSIS_MARKERS:
        if indicator.lower() in lowered:
            findings.append(AnalysisAlert(
                "anti-analysis-scan", "medium", _to_posix(path),
                "Potential anti-analysis / jailbreak marker: '%s'" % indicator,
                "marker." + indicator))
    return findings
...
def _classify_file_worker(task):
    ...
    is_plist_file = lower.endswith((".plist", ".stringsdict"))

    # Force any plist extension file or binary plist signature into structural parsing
    if _is_binary_plist(data) or is_plist_file:
        try:
            ...
            if is_plist_file:
                ...
                result["error"] = "Failed parsing forced plist: " + str(exc)
                return result
...
def classify_and_convert_all(root_dir, plugin_registry=None, max_workers=None):
    ...
    for result in results:
        ...
        if category == FMT_NATIVE_BPLIST:
            if result.get("error"):
                log.warning("Unparseable plist resource, kept byte-identical (%s): %s",
                            rel, result["error"])
                if _write_readable_summary(root_dir, rel, data,
                                            "Property list with a parsing exception",
...
def parse_macho_summary(data):
    ...
    try:
        if bits == 64:
            fields = struct.unpack_from("<IiiIIII", data, 0)
            header_size = 32
            _flags = fields[6]
        else:
            fields = struct.unpack_from("<IiiIII", data, 0)
            header_size = 28
            _flags = fields[5] if len(fields) > 5 else 0
...
_ARM64_CONDTXT = {
    "eq": "==", "ne": "!=", "cs": ">=", "cc": "<",
    "mi": "<", "pl": ">=", "vs": "overflow", "vc": "no-overflow",
    "hi": ">", "ls": "<=", "ge": ">=", "lt": "<",
    "gt": ">", "le": "<=", "al": "true", "nv": "false",
}
...
def _arm64_written_registers(w):
    """Return GPRs written by the ARM64 instruction subset.

    ``_disasm_arm64`` only needs this for invalidating ADRP state.  Returning
    ``None`` for an instruction whose write set is not known is deliberate:
    retaining a stale pointer annotation is worse than losing an annotation
    for one instruction.
    """
    # Branches and other instructions which do not write a GPR.  BL/BLR write
    # the link register, x30.
    if (w >> 26) in (0x05, 0x25):
        return {30} if (w >> 26) == 0x25 else set()
    if (w & 0xFF000010) == 0x54000000:       # B.cond
        return set()
    if (w & 0x7E000000) in (0x34000000, 0x36000000):  # CBZ/TBZ
        return set()
    if (w & 0xFFFFFC1F) in (0xD61F0000, 0xD65F0000):
        return set()                          # BR/RET
    if (w & 0xFFFFFC1F) == 0xD63F0000:
        return {30}                           # BLR
    if (w & 0xFFE0001F) == 0xD4000001:       # SVC
        return set()
    if w == 0xD503201F:                       # NOP
        return set()

    # ADR writes a destination just like ADRP.  ADRP itself is handled by the
    # caller, where its new page value is installed in the state dictionary.
    ...
    # MOVZ/MOVN/MOVK.
    if (w & 0xFF800000) in (
            0x52800000, 0x12800000, 0x72800000,
            0xD2800000, 0x92800000, 0xF2800000):
        rd = w & 0x1F
        return {rd} if rd != 31 else set()

    # Load/store unsigned immediate.  LDR/LDRSW write Rt; STR does not.
    if (w & 0x3F000000) == 0x39000000:
        opc = (w >> 22) & 3
        if opc in (1, 2):
            rt = w & 0x1F
            return {rt} if rt != 31 else set()
        if opc == 0:
            return set()

    # LDP/STP.  LDP writes both register operands, not just Rt.
    if ((w >> 27) & 7) == 5 and not (w & 0x04000000):
        if w & 0x00400000:
            regs = {w & 0x1F, (w >> 10) & 0x1F}
            return {reg for reg in regs if reg != 31}
        return set()

    # ADD/SUB immediate, including the SP forms and CMP/CMN aliases.
    if ((w >> 23) & 0x3F) == 0x22:
        rd = w & 0x1F
        return {rd} if rd != 31 else set()

    # ADD/SUB shifted-register, including CMP.
    if ((w >> 24) & 0x1F) == 0x0B and not (w & 0x00200000):
        rd = w & 0x1F
        return {rd} if rd != 31 else set()

    # Logical shifted-register, including MOV/MVN and TST aliases.
    if ((w >> 24) & 0x1F) == 0x0A:
        rd = w & 0x1F
        return {rd} if rd != 31 else set()

    # The subset decoder does not model every ARM64 encoding.  Clear all
    # tracked values for an unknown instruction so it can never preserve a
    # possibly stale ADRP result.
    return None
...
def _disasm_arm64(code, base_va, max_instructions=20000):
    ...
    Tracks ADRP+ADD pairs targeting the same register so the follow-up ADD
    can annotate the inferred pointer destination (page base + immediate).
    """
    count = min(len(code) // 4, max_instructions)
    adrp_state = {}  # reg -> computed page base
    for k in range(count):
        w = struct.unpack_from("<I", code, k * 4)[0]
        pc = base_va + k * 4
        mnemonic = _decode_arm64(w, pc)

        # ADRP: xd, imm  ->  record page base for xd
        if (w & 0x9F000000) == 0x90000000:
            imm = _sx(((w >> 5) & 0x7FFFF) << 2 | ((w >> 29) & 3), 21) << 12
            adrp_state[w & 0x1F] = (pc & ~0xFFF) + imm
        # ADD (immediate): xd, xn, #imm  -> if xn was ADRP'd and xd == xn,
        # annotate with the calculated pointer destination.
        elif ((w >> 23) & 0x3F) == 0x22 and not (w & 0x40000000):
            rd = w & 0x1F
            rn = (w >> 5) & 0x1F
            if rd == rn and rn in adrp_state and not (w & 0x00400000):
                dest = adrp_state[rn] + ((w >> 10) & 0xFFF)
                mnemonic = "{0}  ; =0x{1:X}".format(mnemonic, dest)
                adrp_state.pop(rn, None)
            else:
                # A non-pairing ADD still writes Rd.  In particular, do not
                # leave an old value alive when Rd == a tracked register.
                writes = _arm64_written_registers(w)
                if writes is None:
                    adrp_state.clear()
                else:
                    for reg in writes:
...
def _pbe_decrypt(data, alg_node, enc_node, password):
    ...
    def _require_blocks(payload, blocksize, label):
        if not payload or len(payload) < blocksize \
                or len(payload) % blocksize:
            raise ValueError(
                "truncated PKCS#12 %s payload: %d bytes (blocksize %d)"
                % (label, len(payload), blocksize))

    if oid == "pbeSha1_3des":
        pk = _der_children(data, params[2], params[3])
        salt = bytes(data[pk[0][2]:pk[0][3]])
        iters = _der_int_at(data, pk[1][2], pk[1][3])
        # Validate the ciphertext before deriving keys or entering the CBC
        # implementation.  CBC decryption must never receive a partial block.
        _require_blocks(enc, 8, "pbeSha1_3des")
    ...
    if oid == "pbes2":
        ...
        if coid == "des3":
            _require_blocks(enc, 8, "pbes2/des3")
        ...
        _require_blocks(enc, 16, "pbes2/" + coid)
...
def inject_dylib_load_command(binary_path, dylib_payload_path,
                              command_id=None):
    """Structurally append a dylib load command to a thin or FAT Mach-O."""
    if command_id is None:
        command_id = REQUESTED_WEAK_DYLIB_CMD
    ...
    if len(original) < 8:
        raise ValueError("binary is truncated or corrupted")

    # Detect FAT/Universal architecture mappings. FAT headers and entries are
    # big-endian; the embedded Mach-O slices remain little-endian.
    fat_magic = struct.unpack_from(">I", original, 0)[0]
    slices = []
    if fat_magic in (0xCAFEBABE, 0xCAFEBABF):
        fat64 = fat_magic == 0xCAFEBABF
        entry_size = 32 if fat64 else 20
        nfat = struct.unpack_from(">I", original, 4)[0]
        off = 8
        for _ in range(min(nfat, 16)):
            if off + entry_size > len(original):
                break
            cputype = struct.unpack_from(">i", original, off)[0]
            if fat64:
                f_off, f_size = struct.unpack_from(">QQ", original, off + 8)
            else:
                f_off, f_size = struct.unpack_from(">II", original, off + 8)
            # ARM64 and x86_64 are the supported 64-bit slice types.
            if cputype in (0x0100000C, 0x01000007):
                if f_size and f_off + f_size <= len(original):
                    slices.append((f_off, f_size))
            off += entry_size
    else:
        slices = [(0, len(original))]

    if not slices:
        raise ValueError("No compatible 64-bit architecture slice found inside the target binary")

    command = bytearray(cmdsize)
    struct.pack_into("<II", command, 0, command_id, cmdsize)
    ...
    patched = []
    for base_off, slice_size in slices:
        slice_end = base_off + slice_size
        if (base_off + 32 > slice_end or
                struct.unpack_from("<I", original, base_off)[0] != 0xFEEDFACF):
            continue

        ncmds, sizeofcmds = struct.unpack_from("<II", original, base_off + 16)
        header_end = base_off + 32 + sizeofcmds
        if header_end > slice_end:
            continue

        first_fileoff = None
        cmd_idx_off = base_off + 32
        for _ in range(min(ncmds, 4096)):
            if cmd_idx_off + 8 > header_end:
                break
            cmd, size = struct.unpack_from("<II", original, cmd_idx_off)
            if size < 8 or cmd_idx_off + size > header_end:
                break
            if cmd == LC_SEGMENT_64 and size >= 72:
                fileoff, filesize = struct.unpack_from("<QQ", original, cmd_idx_off + 32)
                if filesize or fileoff:
                    if fileoff and (first_fileoff is None or fileoff < first_fileoff):
                        first_fileoff = fileoff
            cmd_idx_off += size

        if cmd_idx_off != header_end or first_fileoff is None:
            continue

        relative_header_end = header_end - base_off
        available_slack = first_fileoff - relative_header_end
        if available_slack < cmdsize:
            raise ValueError("Insufficient load-command header padding space inside the binary slice layer")
        if base_off + first_fileoff > slice_end:
            continue

        # Use existing header padding so FAT slice offsets and following slices
        # remain unchanged.
        original[header_end:header_end + cmdsize] = command
        struct.pack_into("<I", original, base_off + 16, ncmds + 1)
        struct.pack_into("<I", original, base_off + 20, sizeofcmds + cmdsize)
        patched.append({
            "offset": base_off,
            "ncmds": ncmds + 1,
            "sizeofcmds": sizeofcmds + cmdsize,
            "slack_before": available_slack,
        })

    if not patched:
        raise ValueError("No compatible 64-bit architecture slice could be patched")
    ...
    with open(tmp, "wb") as fh:
        ...
        try:
            ...
            pass
    _atomic_replace(tmp, binary_path)
    return {"path": binary_path, "status": "Universal slices patched successfully",
            "dylib": name, "command": command_id, "command_size": cmdsize,
            "slices_patched": len(patched), "slices": patched}


def inject_canonical_weak_dylib_load_command(binary_path, dylib_payload_path):
    """Canonical weak-dylib variant using ``LC_LOAD_WEAK_DYLIB | LC_REQ_DYLD``.

    Passes the command ID explicitly to avoid touching the module-level
    ``REQUESTED_WEAK_DYLIB_CMD`` under concurrent callers.
    """
    return inject_dylib_load_command(
        binary_path, dylib_payload_path,
        command_id=LC_LOAD_WEAK_DYLIB | LC_REQ_DYLD)
...
# Entitlements considered too privileged for minimum-context re-signing.
...
def sanitize_entitlements(entitlements_dict, restricted_keys=None):
    """Return a copy of ``entitlements_dict`` with restricted keys removed."""
    if not isinstance(entitlements_dict, dict):
        return entitlements_dict
    if restricted_keys is None:
        restricted_keys = RESTRICTED_ENTITLEMENT_KEYS
    sanitized = dict(entitlements_dict)
    for key in restricted_keys:
        if key in sanitized:
            log.info("Pruning restricted entitlement key: %s", key)
            del sanitized[key]
    return sanitized
...
def _build_signature(body, filetype, identifier, entitlements_xml=None,
    ...
    if real and ent_blob:
        ...
        if isinstance(ent_obj, dict):
            ent_obj = sanitize_entitlements(ent_obj)
            try:
...
def pseudo_sign_bundle(bundle_dir, entitlements_xml=None, depth=0,
    ...
    # Loose Mach-O dylibs in Frameworks are code too. When FridaGadget (or
    # another injected library) is added during a rebuild, sign it before
    # generating CodeResources and before signing the containing executable.
    framework_dir = os.path.join(bundle_dir, "Frameworks")
    if os.path.isdir(framework_dir):
        for filename in sorted(os.listdir(framework_dir)):
            dylib_path = os.path.join(framework_dir, filename)
            if not os.path.isfile(dylib_path) or not filename.endswith(".dylib"):
                continue
            if not _file_is_macho(dylib_path):
                continue
            try:
                with open(dylib_path, "rb") as fh:
                    ddata = fh.read(64 * 1024 * 1024)
                dident = ident
                status = pseudo_sign_binary(dylib_path, dident, entitlements_xml,
                                            None, sign, b"")
                log.info("Signed nested dylib: %s - %s",
                         os.path.relpath(dylib_path,
                                         getattr(pseudo_sign_bundle, "_root", bundle_dir)),
                         status)
                if status.startswith("signed"):
                    signed += 1
            except Exception as exc:
                raise SigningError("Failed to sign Frameworks/%s: %s" %
                                   (filename, exc))
...
def _find_payload_app(framework_dir):
    """Return the single top-level Payload/*.app directory."""
    payload = os.path.join(framework_dir, PAYLOAD_DIR)
    if not os.path.isdir(payload):
        raise FrameworkNotFoundError("Missing Payload directory in framework: %s" % framework_dir)
    apps = [name for name in sorted(os.listdir(payload))
            if name.endswith(APP_SUFFIX) and os.path.isdir(os.path.join(payload, name))]
    if not apps:
        raise FrameworkNotFoundError("No Payload/*.app bundle found in %s" % payload)
    if len(apps) > 1:
        log.warning("Multiple .app bundles found; grafting Frida into %s", apps[0])
    return os.path.join(payload, apps[0])


def _app_executable_name(app_dir):
    """Read CFBundleExecutable, with a conservative filesystem fallback."""
    plist_path = os.path.join(app_dir, "Info.plist")
    if os.path.isfile(plist_path):
        try:
            with open(plist_path, "rb") as fh:
                obj = _plist_loads(fh.read())
            name = obj.get("CFBundleExecutable") if isinstance(obj, dict) else None
            if isinstance(name, _TEXT_TYPES) and name:
                return name
        except Exception as exc:
            log.warning("Could not read CFBundleExecutable from Info.plist: %s", exc)
    # Fallback: common executable basename matching the bundle name.
    candidate = os.path.join(app_dir, os.path.basename(app_dir)[:-len(APP_SUFFIX)])
    if os.path.isfile(candidate):
        return os.path.basename(candidate)
    raise FrameworkNotFoundError("Could not determine CFBundleExecutable in %s" % app_dir)


def _frida_graft_binary(app_dir, gadget_path):
    """Copy Gadget and patch the app's main Mach-O to load it."""
    frameworks = os.path.join(app_dir, "Frameworks")
    _makedirs(frameworks)
    gadget_dst = os.path.join(frameworks, "FridaGadget.dylib")
    shutil.copy2(gadget_path, gadget_dst)
    exe_name = _app_executable_name(app_dir)
    exe_path = os.path.join(app_dir, exe_name)
    if not os.path.isfile(exe_path):
        raise FrameworkNotFoundError("Main executable not found: %s" % exe_path)
    # The canonical weak-dylib command is LC_LOAD_WEAK_DYLIB | LC_REQ_DYLD.
    result = inject_canonical_weak_dylib_load_command(exe_path, gadget_dst)
    return gadget_dst, exe_path, result


def _auto_frida_assets():
    """Locate bundled Frida assets for zero-extra-command builds.

    IPAForge now treats Frida integration as part of normal ``-b``
    recompilation. Assets are searched in predictable locations next to this
    script so Windows users can simply pass the decoded framework folder.
    Returns (gadget_path, script_path), either may be None.
    """
    base = os.path.dirname(os.path.abspath(__file__))
    roots = [
        os.path.join(base, "frida"),
        os.path.join(base, "Frida"),
        os.path.join(base, "frida_assets"),
        base,
    ]
    gadget_names = ("FridaGadget.dylib", "frida-gadget.dylib")
    script_names = ("ipaforge_hooks.js", "frida.js", "hooks.js")
    gadget = None
    script = None
    for root in roots:
        if gadget is None:
        ...
        if script is None:
            for name in script_names:
                ...
                if os.path.isfile(candidate):
                    script = candidate
                    break
        if gadget is not None and script is not None:
            break
    return gadget, script


def automated_frida_graft(framework_dir, gadget_src_path, script_src_path=None):
    """Integrate Frida Gadget into the normal IPAForge build pipeline.

    The Gadget is copied into Payload/<App>.app/Frameworks and the main
    executable is patched in-place to weak-load it. Optionally, a JavaScript
    instrumentation file is copied beside the app resources as
    ``ipaforge_hooks.js``. This function only performs instrumentation setup;
    it does not bypass TLS pinning, jailbreak checks, or other security controls.
    """
    framework_dir = os.path.abspath(framework_dir)
    gadget_src_path = os.path.abspath(gadget_src_path)
    if not os.path.isfile(gadget_src_path):
        raise FrameworkNotFoundError("Frida Gadget not found: %s" % gadget_src_path)
    app_dir = _find_payload_app(framework_dir)
    gadget_dst, exe_path, result = _frida_graft_binary(app_dir, gadget_src_path)
    log.info("Frida Gadget integrated: %s", os.path.relpath(gadget_dst, framework_dir))
    log.info("Patched main executable: %s (LC_LOAD_WEAK_DYLIB=0x%08X)",
             os.path.relpath(exe_path, framework_dir),
             LC_LOAD_WEAK_DYLIB | LC_REQ_DYLD)
    script_dst = None
    if script_src_path:
        script_src_path = os.path.abspath(script_src_path)
        if not os.path.isfile(script_src_path):
            raise FrameworkNotFoundError("Frida script not found: %s" % script_src_path)
        script_dst = os.path.join(app_dir, "ipaforge_hooks.js")
        shutil.copy2(script_src_path, script_dst)
        log.info("Frida script embedded: %s", os.path.relpath(script_dst, framework_dir))
    return {"app": app_dir, "gadget": gadget_dst, "executable": exe_path,
            "script": script_dst, "injection": result}
...
def build_ipa(input_dir, output_path=None, force=False, compression_level=9,
              ...
              frida_inject=None, frida_script=None, auto_frida=True):
    ...
    # Force structural Frida dynamic loading command architecture injection
    if auto_frida:
        auto_gadget, auto_script = _auto_frida_assets()
        if frida_inject is None:
            frida_inject = auto_gadget
        if frida_script is None:
            frida_script = auto_script

        # Programmatically compile a valid micro-dylib target inside the tool if absent on disk
        if frida_inject is None or not os.path.isfile(frida_inject):
            log.warning("FridaGadget.dylib source file absent from asset paths.")
            log.info("Compiling internal 64-bit LE micro-dylib structure inside the tool.")
            stub_dir = os.path.join(input_dir, "readable", "stubs")
            _makedirs(stub_dir)
            frida_inject = os.path.join(stub_dir, "FridaGadget.dylib")

            if not os.path.isfile(frida_inject):
                # Assemble a syntactically accurate 256-byte 64-bit LE ARM64 Mach-O dylib payload
                # Satisfies magic, size bounds, sizeofcmds, and __TEXT/__LINKEDIT slice paths
                payload = bytearray(0x100)

                # 1. Pack Mach-O Header 64 (32 bytes):
                # magic (0xFEEDFACF), cputype (ARM64 = 0x0100000C), cpusubtype (0),
                # filetype (MH_DYLIB = 6), ncmds (2), sizeofcmds (144), flags (0), reserved (0)
                struct.pack_into("<IiiIIIII", payload, 0, 0xFEEDFACF, 0x0100000C, 0, 6, 2, 144, 0, 0)

                # 2. Pack Load Command 1: LC_SEGMENT_64 for __TEXT (72 bytes) at offset 32:
                # cmd (0x19), cmdsize (72), segname (__TEXT), vmaddr (0), vmsize (0x100),
                # fileoff (0), filesize (0x100)
                struct.pack_into("<II16sQQQQiiII", payload, 32, 0x19, 72, b"__TEXT\x00", 0, 0x100, 0, 0x100, 7, 5, 0, 0)

                # 3. Pack Load Command 2: LC_ID_DYLIB (72 bytes) at offset 104:
                # cmd (0xD), cmdsize (72), name_offset (24), timestamp (0),
                # current_ver (0x10000), compat_ver (0x10000)
                struct.pack_into("<IIIIII", payload, 104, 0xD, 72, 24, 0, 0x10000, 0x10000)
                # Append the local install library navigation path string name reference
                path_str = b"@rpath/FridaGadget.dylib\x00"
                payload[128:128 + len(path_str)] = path_str

                with open(frida_inject, "wb") as fh:
                    fh.write(bytes(payload))

        log.info("Executing mandatory Frida load command structure grafting...")
        try:
            automated_frida_graft(input_dir, frida_inject, frida_script)
            log.info("Frida structural loading commands applied flawlessly.")
        except (IPAForgeError, OSError, ValueError) as exc:
            raise SigningError("Mandatory Frida structure command injection failed: %s" % exc)
...
def build_arg_parser():
    ...
    signing.add_argument(
        ...
        default=None,
        metavar="FILE",
        help="(build) integrate a local FridaGadget.dylib into the app "
             "during recompilation, before CodeResources/signing",
    )
    signing.add_argument(
        "--frida-script",
        dest="frida_script",
        default=None,
        metavar="FILE",
        help="(build) embed a Frida JavaScript instrumentation file as "
             "Payload/<App>.app/ipaforge_hooks.js",
    )
    signing.add_argument(
        "--no-frida",
        dest="auto_frida",
        action="store_false",
        default=True,
        help="(build) disable the automatic Frida Gadget/script integration",
    )
    ...
    try:
        if args.mode == MODE_DECODE:
            ...
            build_ipa(
                ...
                frida_inject=getattr(args, "frida_inject", None),
                frida_script=getattr(args, "frida_script", None),
                auto_frida=getattr(args, "auto_frida", True),
            )
...
def _self_test():
    ...
    with tempfile.TemporaryDirectory(prefix="ipaforge-test-") as td:
        ...
        # An ADD which does not form the pair must still invalidate its
        # destination, and LDP must invalidate both of its destinations.
        arm64_clobber = list(_disasm_arm64(struct.pack(
            "<4I", 0x90000000, 0xD2800020, 0x91000800, 0xD503201F), 0x1000))
        assert arm64_clobber[2].mnemonic == "add x0, x0, #0x2"
        ldp = 0xA9400000 | (1 << 10) | (2 << 5)
        arm64_pair = list(_disasm_arm64(struct.pack(
            "<4I", 0x90000000, 0x90000001, ldp, 0x91000800), 0x1000))
        assert "; =" not in arm64_pair[3].mnemonic

        # CBC input must be rejected before the KDF/decrypt implementation.
        pbe_alg = _der_seq(
            _der_oid(OID["pbeSha1_3des"]),
            _der_seq(_der_octet(b"salt"), _der_int(1)))
        pbe_enc = _der_octet(b"1234567")
        pbe_data = bytearray(pbe_alg + pbe_enc)
        pbe_nodes = _der_children(pbe_data, 0, len(pbe_data))
        try:
            _pbe_decrypt(pbe_data, pbe_nodes[0], pbe_nodes[1], "password")
        except ValueError as exc:
            assert "truncated PKCS#12" in str(exc)
        else:
            raise AssertionError("unaligned PKCS#12 payload was accepted")
