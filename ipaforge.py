...
def build_ipa(input_dir, output_path=None, force=False, compression_level=9,
    ...
    # Frida integration is part of normal recompilation.  With no explicit
    # --frida-* arguments, -b automatically discovers the bundled Gadget/script
    # next to this program (or in ./frida) and grafts them before CodeResources,
    # hashing, and final signing.  This makes Windows folder builds one-step.
    if auto_frida:
        ...
        if frida_inject is not None:
            log.info("Automatic Frida integration enabled for this build")
            try:
                automated_frida_graft(input_dir, frida_inject, frida_script)
            except (IPAForgeError, OSError, ValueError) as exc:
                raise SigningError("Frida integration failed: %s" % exc)
        else:
            log.info("No bundled FridaGadget.dylib found - building without Frida")
